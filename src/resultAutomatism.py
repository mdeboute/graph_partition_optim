...
def do(path,k,neighborhoodSize,iterMax,timeOut,tabuSize) :
    ...
    if doGradient :
        t = time.time()
        sol,solCost = gradientDescent( initialSolution, initialSolutionCost, timeOut, False, neighborhoodSize)
        print("\nGD found the solution : ",sol," with a cost of ",solCost, " in ", time.time()-t,"seconds.")
        
    if doBetterGradient :
        t = time.time()
        sol,solCost = betterGradientDescent( initialSolution, initialSolutionCost, timeOut, False, neighborhoodSize)
